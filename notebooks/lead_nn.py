...
# fc_w_2 = tf.get_variable('fcw_2', shape=[fc_a_1.shape.as_list()[1], n_hidden_units[2]], dtype=tf.float32, initializer=tf.contrib.layers.xavier_initializer(seed=seed))
# fc_z_2 = tf.matmul(fc_a_1, fc_w_2)
# fc_a_2 = tf.nn.relu(fc_z_2)
...
#pred = tf.multiply(H[:, 18:], tf.matmul(fc_a_1, w_out), name='pred')
#pred = tf.multiply(H[:, 18:], tf.nn.sigmoid(tf.matmul(fc_a_1, w_out)))
...
labels = tf.nn.relu(-1 * (C - 1))  # this line just sets the DD-optimal cards to 1 and alle else to 0

#cost_pred = tf.reduce_sum(tf.squared_difference(pred, labels))
#cost_pred = tf.reduce_sum(tf.multiply(pred, C))
cost_pred = tf.reduce_sum(tf.nn.sigmoid_cross_entropy_with_logits(logits=pred_logit, labels=labels))
